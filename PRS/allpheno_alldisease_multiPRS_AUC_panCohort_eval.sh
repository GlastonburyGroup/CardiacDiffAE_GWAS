...
#SBATCH --output=evalAUCprs_%x_%j.log
...
function usage {
    echo ""
    echo "Runs eval_multiPRS_V2_wCovar.py for both diagnosis and prognosis (1-to-10y),"
    echo "generating AUC box plots (SF27A/SF27B) and exporting the corresponding TSVs."
    echo "All other plot types are disabled by default."
    echo ""
    echo "usage: $programmename [--out_tag_diag string] [--out_tag_prog string] ..."
    echo ""
}
...
#diagnosis-specific
out_tag_diag="${out_tag_diag:-panCohortV2_NyDiag_auto_lw_gw_10kIT_kingB4ldpred2}"
disease_root_diag="${disease_root_diag:-/project/ukbblatent/clinicaldata/binary_disease_cohorts/F20208v3_nonDiscov/caucasian_king0p0625_grouped/newcovsets/V0v2}"

#prognosis-specific (override disease_root_prog if prognosis cohorts differ)
out_tag_prog="${out_tag_prog:-panCohortV2_1to10yProg_auto_lw_gw_10kIT_kingB4ldpred2}"
disease_root_prog="${disease_root_prog:-$disease_root_diag}"
...
#obtain_summary=1 is required: it writes combined_results.tsv that the TSV export reads
obtain_summary="${obtain_summary:-1}"
obtain_pairwise_improvements="${obtain_pairwise_improvements:-0}"

#AUC box plot only; all other plot types off
plot_box_AUC="${plot_box_AUC:-1}"
...
plot_prevalence_prs_pancohort="${plot_prevalence_prs_pancohort:-0}"
plot_prevalence_prob_pancohort="${plot_prevalence_prob_pancohort:-0}"
plot_cum_disease_burden="${plot_cum_disease_burden:-0}"
plot_cum_hazard="${plot_cum_hazard:-0}"
plot_KM_survival="${plot_KM_survival:-0}"
plot_cox_ph="${plot_cox_ph:-0}"
dis_plots_rawPRS="${dis_plots_rawPRS:-0}"
...
export_out_dir="${export_out_dir:-$output_root/paper_tsvs_AUC}"
top_n_diseases="${top_n_diseases:-25}"
...
#Helper function: runs the eval python script with the shared flags.
#Args: $1 = out_tag, $2 = disease_root
run_eval() {
    local out_tag_arg="$1"
    local disease_root_arg="$2"
    $conda/bin/python $programme \
        --output_root "$output_root/$out_tag_arg" \
        --disease_root "$disease_root_arg" \
        ...
        --n_top_dis "$top_n_diseases"
}

echo "=== Running DIAGNOSIS eval ==="
run_eval "$out_tag_diag" "$disease_root_diag" &

echo "=== Running PROGNOSIS (1-to-10y) eval ==="
run_eval "$out_tag_prog" "$disease_root_prog"

if [[ "$run_export" -eq 1 ]]; then
    echo "=== Exporting performance metrics TSVs ==="
    $conda/bin/python $export_programme \
        --output_root_diagnosis "$output_root/$out_tag_diag" \
        --output_root_prognosis "$output_root/$out_tag_prog" \
        $( [[ "$adjust_prs_for_covars" -eq 1 ]] && echo '--covar_adjusted' || echo '--no-covar_adjusted' ) \
        --no-export_disease_plots \

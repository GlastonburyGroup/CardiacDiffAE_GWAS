...
# %%
def fit_logistic_regression(data, trainIIDs, testIIDs, IIDs_nodisease=None, x_var='PRS_Score', col_y='BinCAT_Disease', cont_covars=None, cat_covars=None):
...
def fit_lasso(data, trainIIDs, testIIDs, prs_cols, IIDs_nodisease=None, col_y='BinCAT_Disease', cont_covars=None, cat_covars=None, scale_prs=False, max_iter=1000, random_state=42, threads=5):
    data_train = data[data.index.isin(trainIIDs)]
    data_test = data[data.index.isin(testIIDs)]
...
if __name__ == "__main__":
    ...
    for fold, (train_index, test_index) in tqdm(enumerate(kf.split(IIDs), 1)):
        ...
        #multiPRS
        if args.do_multiPRS:
            res_store[f"Fold_{fold}"]['Lasso']['multiPRS'] = fit_lasso(Xy, trainIIDs, testIIDs, IIDs_nodisease=IIDs_nodisease, prs_cols=prs_cols, col_y=args.col_disease, scale_prs=False, max_iter=args.lassoCV_max_iter, random_state=42, threads=args.threads)

        #multiPRS normalised
        if args.do_multiPRSNorm:
            res_store[f"Fold_{fold}"]['Lasso']['multiPRSNorm'] = fit_lasso(Xy, trainIIDs, testIIDs, IIDs_nodisease=IIDs_nodisease, prs_cols=prs_cols, col_y=args.col_disease, scale_prs=True, max_iter=args.lassoCV_max_iter, random_state=42, threads=args.threads)

        #multiPRS + covar
        if args.do_multiPRSCovar:
            res_store[f"Fold_{fold}"]['Lasso']['multiPRSCovar'] = fit_lasso(Xy, trainIIDs, testIIDs, IIDs_nodisease=IIDs_nodisease, prs_cols=prs_cols, cont_covars=args.cont_covar_cols, cat_covars=args.cat_covar_cols, col_y=args.col_disease, scale_prs=False, max_iter=args.lassoCV_max_iter, random_state=42, threads=args.threads)
            
            #without PC covars
            if args.do_nonPCCovar and bool(args.nPCs_covar):
                res_store[f"Fold_{fold}"]['Lasso']['multiPRSnonPCCovar'] = fit_lasso(Xy, trainIIDs, testIIDs, IIDs_nodisease=IIDs_nodisease, prs_cols=prs_cols, cont_covars=nonPC_cont_covar_cols, cat_covars=args.cat_covar_cols, col_y=args.col_disease, scale_prs=False, max_iter=args.lassoCV_max_iter, random_state=42, threads=args.threads)

        #multiPRS + covar
        if args.do_multiPRSNormCovar:
            res_store[f"Fold_{fold}"]['Lasso']['multiPRSNormCovar'] = fit_lasso(Xy, trainIIDs, testIIDs, IIDs_nodisease=IIDs_nodisease, prs_cols=prs_cols, cont_covars=args.cont_covar_cols, cat_covars=args.cat_covar_cols, col_y=args.col_disease, scale_prs=True, max_iter=args.lassoCV_max_iter, random_state=42, threads=args.threads)
            
            #without PC covars
            if args.do_nonPCCovar and bool(args.nPCs_covar):
                res_store[f"Fold_{fold}"]['Lasso']['multiPRSNormnonPCCovar'] = fit_lasso(Xy, trainIIDs, testIIDs, IIDs_nodisease=IIDs_nodisease, prs_cols=prs_cols, cont_covars=nonPC_cont_covar_cols, cat_covars=args.cat_covar_cols, col_y=args.col_disease, scale_prs=True, max_iter=args.lassoCV_max_iter, random_state=42, threads=args.threads)

        #covar
        if args.do_covar:
            res_store[f"Fold_{fold}"]['Lasso']['covar'] = fit_lasso(Xy, trainIIDs, testIIDs, IIDs_nodisease=IIDs_nodisease, prs_cols=[], cont_covars=args.cont_covar_cols, cat_covars=args.cat_covar_cols, col_y=args.col_disease, scale_prs=False, max_iter=args.lassoCV_max_iter, random_state=42, threads=args.threads)
            
            #without PC covars
            if args.do_nonPCCovar and bool(args.nPCs_covar):
                res_store[f"Fold_{fold}"]['Lasso']['nonPCCovar'] = fit_lasso(Xy, trainIIDs, testIIDs, IIDs_nodisease=IIDs_nodisease, prs_cols=[], cont_covars=nonPC_cont_covar_cols, cat_covars=args.cat_covar_cols, col_y=args.col_disease, scale_prs=False, max_iter=args.lassoCV_max_iter, random_state=42, threads=args.threads)

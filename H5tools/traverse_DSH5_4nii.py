...
# parser.add_argument("--dataH5", default="data.h5", help="name of the HDF5 file")
parser.add_argument("--mode", type=int, default=8, help="0: fetch all the subject IDs, " \
...
parser.add_argument("--export_N", type=int, default=100, help="export only N images, default is 100. This is only used when the mode is 8. If 0 or -1 or None, all images are exported")
parser.add_argument("--export_key", type=str, default="primary", help="export key for the images, default is blank = use all.")

parser.add_argument("--out_path", default="/group/glastonbury/soumick/dataset/ukbbnii/minisets/", help="path where the outputs will be stored (subfolder with the final folder name of the in_path will be created). If blank, the in_path will be used.")
parser.add_argument("--out_export_tag", type=str, default="exported_images", help="Name of the subfolder where the exported images will be stored when mode is 8. Default is 'exported_images'. This will be created inside the out_path.")
...
if args.save_findings:
    if args.mode == 8:
        os.makedirs(f"{args.out_path}/{args.out_export_tag}/{args.export_format}", exist_ok=True)
    else:
        ...
        def export_image(name, obj):
            ...
            if isinstance(obj, h5py.Dataset):
                ...
                if export_format in ["NIFTI", "NII"]:
                    ...
                    nib.save(img, f"{args.out_path}/{args.out_export_tag}/{args.export_format}/{name.replace('/', '_')}.nii.gz")
                    ...
                    if ndim == 2:
                        ...
                        img.save(f"{args.out_path}/{args.out_export_tag}/{args.export_format}/{name.replace('/', '_')}.{ext}")
                    elif ndim == 3:
                        # Save as multi-page TIFF or GIF
                        imgs = [Image.fromarray(data.squeeze()[...,i]) for i in range(data.shape[-1])]
                        ext = "tiff" if export_format in ["TIFF", "TIF"] else "gif"
                        imgs[0].save(
                            f"{args.out_path}/{args.out_export_tag}/{args.export_format}/{name.replace('/', '_')}.{ext}",
                        ...
                        for index in idx:
                            ...
                            img.save(f"{args.out_path}/{args.out_export_tag}/{args.export_format}/{name.replace('/', '_')}_{idx_str}.{ext}")
                else:
                    ...
                    if ndim == 2:
                        ...
                        img.save(f"{args.out_path}/{args.out_export_tag}/{args.export_format}/{name.replace('/', '_')}.{export_format.lower()}")
                    else:
                        idx = np.ndindex(data.shape[:-2])
                        for index in idx:
                            slice2d = data[index]
                            ...
                            idx_str = "_".join([f"d{i}_{v}" for i, v in enumerate(index)])
                            img = Image.fromarray(scaled)
                            img.save(f"{args.out_path}/{args.out_export_tag}/{args.export_format}/{name.replace('/', '_')}_{idx_str}.{export_format.lower()}")
                counter += 1

...
# parser.add_argument("--in_path", help="path to store the HDF5 file", default=r"/scratch/glastonbury/datasets/ukbbH5s/F20204_Liver_Imaging_T1_ShMoLLI_DICOM_H5v3")
...
# parser.add_argument("--mask_file", help="name of the mask file located inside <in_path> (default: meta_mask.h5)", default="meta_mask_data_ftSAM_liver_Liver.h5")
parser.add_argument("--mask_primary_only", action=argparse.BooleanOptionalAction, help="Whether the mask file contains mask for primary (that is to be applied to all dataset types) or available for each dataset seperately.", default=False)
parser.add_argument("--acq_filter", default="", help="Which acquisition to use (For CardiacDiffAE paper: LAX_4Ch_transverse)")
...
# parser.add_argument("--only_ROI", action=argparse.BooleanOptionalAction, help="If True, then only the ROI will be kept. If False, a contour around the ROI will be created and everything side that box will be kept.", default=True)
# parser.add_argument("--shapeX", type=int, help="Desired shape X", default=224)
# parser.add_argument("--shapeY", type=int, help="Desired shape Y", default=256)
args = parser.parse_args()

if args.in_path.endswith('.h5'):
    in_dir = os.path.splitext(args.in_path)[0]
    in_file = os.path.basename(args.in_path)
else:
    in_dir = args.in_path
    in_file = "data.h5"
out_path = in_dir
...
if args.mask_file != "meta_mask.h5":
    out_path += f"_{os.path.basename(args.mask_file).replace('.h5','').replace('meta_mask_','').replace(in_file.replace('.h5','')+'_','')}"
...
cropped_file = h5py.File(f"{out_path}/{in_file}", 'w')
...
def _crop_with_mask_single(name, obj):
    tag = name.split("/")[-1]
    if args.mask_primary_only and not tag.startswith("primary"):
        # Build the corresponding primary tag by stripping the type and uppercase subtype parts
        parts = tag.split('_')
        i = 1  # Skip the type (e.g., 'auxiliary')
        while i < len(parts) and parts[i].isupper():
            i += 1
        remaining = parts[i:]
        primary_tag = 'primary' + ('_' + '_'.join(remaining) if remaining else '')
        mask_name = '/'.join(name.split('/')[:-1]) + '/' + primary_tag
    else:
        mask_name = name

    if mask_name not in mask_file:
        print(f"Skipping {name}: mask key '{mask_name}' not found in mask file")
        return

    contour = mask_file[mask_name][:]

    if not np.any(contour):
        print(f"Skipping {name}: empty mask (no nonzero values)")
        return

...
from complex_linears import ComplexLinear
...
class MLAnalyses(AnalysersDataHandler):
    def __init__(self, args):
        ...
        self.X = self.df[self.latent_factors] + 0j
        self.X_heldout = self.df_test if self.df_test is None else self.df_test[self.latent_factors] + 0j
    ...
    def eval_linear(self, model, features, X_train, y_train, X_test=None, y_test=None, exp_type="", level_type="", complex_eval=False):
        ...
        if complex_eval:
            ...
            coef_combined = np.concatenate((model.coef_.real, model.coef_.imag))
            features = [f"{f}_real" for f in features] + [f"{f}_imag" for f in features]
            coefficients = pd.DataFrame(coef_combined, index=features, columns=['Coefficient'])

            x_combined = np.concatenate((np.real(X_train), np.imag(X_train)), axis=1)
            columns_real = [f"{col}_real" for col in X_train.columns]
            columns_imag = [f"{col}_imag" for col in X_train.columns]
            X_train = pd.DataFrame(x_combined, columns=columns_real + columns_imag)
    ...
    def eval_logistic(self, model, features, X_train, X_test=None, y_test=None, is_binary=False, exp_type="", level_type="", complex_eval=False):
        if X_test is not None:
            y_pred = model.predict(X_test)
            if bool(self.out_path) and bool(exp_type):
        ...
        if complex_eval:
            coefficients = pd.DataFrame(model.coef_, index=features, columns=classes)
        else:
            coef_combined = np.concatenate((model.coef_.real, model.coef_.imag))
            features = [f"{f}_real" for f in features] + [f"{f}_imag" for f in features]
            coefficients = pd.DataFrame(coef_combined, index=features, columns=classes)
    ...
    def single_linear_regression(self, X_train, X_test, y_train, y_test, exp_type, level_type):
        model = ComplexLinear(X_train.shape[1])
        model.fit_regressor(X_train, y_train)

        self.eval_linear(model=model, features=self.latent_factors, X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test, exp_type=f"CVeval_{exp_type}", level_type=level_type, complex_eval=True)
        self.eval_linear(model=model, features=self.latent_factors, X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test, exp_type=f"RVeval_{exp_type}", level_type=level_type, complex_eval=False)
    ...
    def single_logistic_regression(self, X_train, X_test, y_train, y_test, is_binary, exp_type, level_type):
        ...
        self.eval_logistic(model=model, features=self.latent_factors, X_train=X_train, X_test=X_test, y_test=y_test, is_binary=is_binary, exp_type=f"CVeval_{exp_type}", level_type=level_type, complex_eval=True)
        self.eval_logistic(model=model, features=self.latent_factors, X_train=X_train, X_test=X_test, y_test=y_test, is_binary=is_binary, exp_type=f"RVeval_{exp_type}", level_type=level_type, complex_eval=False)
    ...
    def lasso(self, X_train, X_test, y_train, y_test, exp_type, level_type, l1_pen=None):
        model = ComplexLinear(X_train.shape[1], lambda_reg=l1_pen)
        model.fit_regressor(X_train, y_train)
    ...
    def save_results(self, flush_results=False):
        if bool(self.out_path):
            ...
            with open(os.path.join(self.out_path, f"CVevalV0_{self.tsv_name}_results.pkl"), "wb") as f:
    ...
    def scale_features(self, X_train, X_test):
        sys.exit("Not yet supported for complex data")
...
def process_arguments():
    ...
    # parser.add_argument('--embH5', action="store", default="/group/glastonbury/soumick/out/Results/F20259v3_Pancreas_1stAEfold0_prec32_pythaemodel-ae/Output_fullDS200ep", help="Path of the raw emb.h5 file containining the results of the model (can be only just the folder containing emb.h5)")
    parser.add_argument('--embH5', action="store", default="/group/glastonbury/soumick/out/Results/F20259v3_Pancreas_259L1_lr03_2D_baseinit1fold0_prec32_pythaemodel-custom_complex_ae/Output_fullDS200ep", help="Path of the raw emb.h5 file containining the results of the model (can be only just the folder containing emb.h5)")
    ...
    parser.add_argument('--cov_path', action="store", default="/group/glastonbury/GWAS/inputs/covariates/F20259_Pancreas_Images_ShMoLLI_DICOM_H5v3/cov_F20259_Pancreas_Images_ShMoLLI_DICOM_H5v3_woSmoking_MultiVisits.tsv", help="Fully-qualified path of the tsv file containining the basic covariates.")
    ...
    parser.add_argument('--out_path', action="store", default="/project/ukbblatent/soumick/ML_analyses/F20259v3/v1.1.0_seventh_basket/CAE_Rupali", help="Path to store the results. Make it blank if if it's desired to not store the results, rather just print them on the console.")
    ...
    parser.add_argument('--use_feature_scaling', action=argparse.BooleanOptionalAction, default=False, help="[Does not work for complex yet, it's just a placeholder for now] Whether to use feature scaling or not.")
...
def main():
    ...
    for tsv_file_combo in tsv_file_combos:
        ...
        for target_attribute in analyser.attributes:
            process_attribute(target_attribute, analyser, args, None)

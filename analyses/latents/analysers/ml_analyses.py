...
from utils.maths import safe_exp, safe_log, safe_sqrt
...
class MLAnalyses(AnalysersDataHandler):
    ...
    def eval_linear(self, model, features, X_train, y_train, X_test=None, y_test=None, exp_type="", level_type=""):
        ...
        mse = np.sum(residuals**2) / (dof + np.finfo(np.float32).eps)

        t_values, p_values = [], []
        for i, coef in enumerate(coefficients['Coefficient']):
            standard_error = safe_sqrt(mse / (np.sum((X_train.iloc[:, i] - X_train.iloc[:, i].mean())**2)) + np.finfo(np.float32).eps)
            t_stat = coef / (standard_error + np.finfo(np.float32).eps)
    ...
    def eval_logistic(self, model, features, X_train, X_test=None, y_test=None, is_binary=False, exp_type="", level_type=""):
        ...
        for cls in classes:
            coefficients[cls.replace("Coefficient", "OddsRatio")] = safe_exp(coefficients[cls]) # Calculate odds ratios
            ...
            XTX += np.finfo(np.float32).eps * np.eye(XTX.shape[0])  # Regularise to avoid singular matrices
            standard_errors = X_std * safe_sqrt(np.diag(np.linalg.inv(XTX)))

            # Calculate the confidence intervals
            z = stats.norm.ppf(1 - 0.05 / 2)
            coefficients[cls.replace("Coefficient", "LowerCI")] = safe_exp(coefficients[cls] - z * standard_errors)
            coefficients[cls.replace("Coefficient", "UpperCI")] = safe_exp(coefficients[cls] + z * standard_errors)
...
def process_arguments():
    ...
    # To work directly with the emb.h5 file
    parser.add_argument('--embH5', action="store", default="/project/ukbblatent/Out/Results/F20208_heart_1Ses_time2slc_MskCrop128_mulseed_1701_3D_128fold0_prec16-mixed_DiffAE/Output_fullDSV3", help="Path of the raw emb.h5 file containining the results of the model (can be only just the folder containing emb.h5)")
    ...
    parser.add_argument('--all_datatags', action=argparse.BooleanOptionalAction, default=True, help="Whether or not to consider all the data tags, or only the first one for each subject.")

    #To work with the processed tsv files (including the merged ones)
    parser.add_argument('--res_path', action="store", default="", help="(Optional) Fully-qualified path of the tsv file containining the results of the model (e.g., processed_raw.tsv)")
    ...
    parser.add_argument('--cov_path', action="store", default="/group/glastonbury/GWAS/inputs/covariates/F20208_Long_axis_heart_images_DICOM_H5v3/cov_F20208_Long_axis_heart_images_DICOM_H5v3_woSmoking_woMRICentre.tsv", help="Fully-qualified path of the tsv file containining the basic covariates.")
    parser.add_argument("--cov_cols", action="store", default="BMI,Sex,Age,MRI_Date", help="Comma-separated list of the (correction) covariates for the association analysis. Leave it blank if association analysis is not desired.")
    ...
    parser.add_argument("--cov_cat", action="store", default="MRI_Date", help="Comma-separated list of the (correction) categorical covariates for the association analysis. Leave it blank if association analysis is not desired.")
        
    parser.add_argument('--model_tag', action="store", default="DiffAE_F20208_heart_Gen1_MskCrop128_mulseed_1701_3D", help="Tag to identify the model.")

    parser.add_argument('--out_path', action="store", default="/project/ukbblatent/soumick/ML_analyses/Atlas/MultiOrganV3_Initial/heart/PhecodeCh/diag10y", help="Path to store the results. Make it blank if if it's desired to not store the results, rather just print them on the console.")
    ...
    parser.add_argument('--is_disease', action=argparse.BooleanOptionalAction, default=True, help="Whether the supplied TSVs are of disease (if False, they will be considered to be measurements.) [To be considered if instance_merge is True]")
    parser.add_argument('--path_tsvs', action="store", default="/project/ukbblatent/clinicaldata/Atlas/binary_disease_cohorts/MultiOrganV3/ignore_mismatch_inst/heart/PhecodeCh/diag10y", help="Location of the tsv files, to be used for analyses. Leave it blank if fully-qualified paths are supplied in --tsv_files.")
    # parser.add_argument('--path_tsvs', action="store", default="/project/ukbblatent/clinicaldata/abdominalMRI", help="Location of the tsv files, to be used for analyses. Leave it blank if fully-qualified paths are supplied in --tsv_files.")
    parser.add_argument('--tsv_files', action="store", default="", help="Coma-separated list of tsv [or even csv] files - to be joined and considered. To consider in one analysis, seperate by coma, to be considered in seperate ones, seperate by semicolon.")
    parser.add_argument('--index_col', action="store", default="FID", help="name of the column that contains the subject IDs, to be used as indices.")
    parser.add_argument('--tsvcols2consider', action="store", default="", help="Comma-speparated list of columns from the TSV files to consider for the analysis.")
    ...
    parser.add_argument('--use_target_scaling', action=argparse.BooleanOptionalAction, default=False, help="Whether to use target scaling or not (continuous targets only).")
    parser.add_argument('--remove_with_VIF', action=argparse.BooleanOptionalAction, default=False, help="Whether to remove VIFs.")

    parser.add_argument('--perform_featsel', action=argparse.BooleanOptionalAction, default=True, help="Perform additional analysis with feature selection.")
    parser.add_argument('--perform_RFE', action=argparse.BooleanOptionalAction, default=False, help="Perform additional analysis with Recursive Feature Elimination [Warning: time-consuming].")
    ...
    parser.add_argument('--n_jobs', action="store", type=int, default=6, help="Number of jobs to use during modelling.")
    parser.add_argument('--n_parallel', action="store", type=int, default=1, help="Number of jobs to use for parallelisation over the attributes.")
...
def main():
    ...
    if bool(args.tsv_files):
        tsv_file_combos = args.tsv_files.split(';')
    elif bool(args.path_tsvs):
        tsv_file_combos = glob(os.path.join(args.path_tsvs, "*.tsv")) + glob(os.path.join(args.path_tsvs, "*.csv"))
        args.path_tsvs = ""
    ...
    for tsv_file_combo in tsv_file_combos:
        ...
        for complex_mode in args.complex_modes:
            ...
            parallel_process_attributes(analyser, args, num_threads=1)

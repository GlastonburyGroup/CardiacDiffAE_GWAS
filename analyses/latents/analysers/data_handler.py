...
class AnalysersDataHandler:
    def __init__(self, args):
        ...
        if bool(self.embH5):
            ...
            if self.df['data_tag'].nunique() > 1 and not self.all_datatags:
                selected_tag = self.df['data_tag'].value_counts().sort_values(ascending=False).index[0]
                print(f"Multiple datatags found inside embeddings file, but all_datatags is set to False. Hence, the tag with the most subjects has been chosen: {selected_tag}")
                self.df = self.df[self.df['data_tag'] == selected_tag]
            elif self.df['data_tag'].nunique() == 1 and self.all_datatags:
                self.all_datatags = False
        ...
        cols2keep = copy(self.latent_factors)
        self.indcols = ["subID"]
        if bool(self.embH5):
            if self.instance_merge:
                cols2keep += ["instanceID"]
                self.indcols += ["instanceID"]
            if self.all_datatags:
                cols2keep += ["data_tag"]
                self.indcols += ["data_tag"]
        self.df = self.df[cols2keep]
        if self.df_test is not None:
            self.df_test = self.df_test[cols2keep]
        ...
        if bool(self.cov_path) and bool(self.cov_cols):
            ...
            self.df = self.df.reset_index().rename({"index":"subID"}, axis=1).merge(df_temp, left_on=['subID', 'instanceID'], right_on=['IID_COV', 'MRI_Visit_COV'], how='inner').drop(columns=['IID_COV', 'MRI_Visit_COV']).set_index(self.indcols) 
            self.df_test = self.df_test.reset_index().rename({"index":"subID"}, axis=1).merge(df_temp, left_on=['subID', 'instanceID'], right_on=['IID_COV', 'MRI_Visit_COV'], how='inner').drop(columns=['IID_COV', 'MRI_Visit_COV']).set_index(self.indcols) if self.df_test is not None else None            
            self.cov_cols = []
            for att in attribs:
        ...
        for tsv_file in self.tsv_files:
            if tsv_file.endswith('.csv'):
                df_temp = pd.read_csv(f"{self.path_tsvs}/{tsv_file}" if bool(self.path_tsvs) else tsv_file)
            else:
                tsv_file = tsv_file if tsv_file.endswith('.tsv') else f"{tsv_file}.tsv"
                df_temp = pd.read_table(f"{self.path_tsvs}/{tsv_file}" if bool(self.path_tsvs) else tsv_file)
            if args['index_col'] != "f.eid":
                df_temp = df_temp.rename(columns={args['index_col']: "f.eid"})  
            df_temp = df_temp.set_index('f.eid')
            df_temp.index = df_temp.index.astype('int32')
            if bool(self.tsvcols2consider):
                self.tsvcols2consider = self.tsvcols2consider.split(',')
                if self.instance_merge:
                    self.tsvcols2consider.append("MRI_Visit" if self.is_disease else "instanceID")
                df_temp = df_temp[self.tsvcols2consider]
            df_temp = reduce_numeric_precision(df_temp)

            if self.instance_merge:
                if self.is_disease:
                    df_temp.rename(columns={"MRI_Visit": "instanceID"}, inplace=True)
                else:
                    df_temp = correct_errors_raw(df_temp)
                    df_temp = df_temp.reset_index().melt(id_vars='f.eid', var_name='measurement', value_name='value')
                    df_temp['instanceID'] = (df_temp['measurement'].str.extract(r'\.(\d+\.\d+)$')[0]
                                                                    .str.replace('.0', '', regex=False)
                                                                    .astype("int8") ) #let's assume there is no subtype of the instance
                ...
                df_reset = self.df.reset_index().set_index([c for c in self.indcols if c != "data_tag"])
                df_temp = df_temp.reset_index().rename({"f.eid":"subID"}, axis=1).set_index([c for c in self.indcols if c != "data_tag"])
                self.df = df_reset.join(df_temp, how="inner").reset_index().set_index(self.indcols)
                if self.df_test is not None:
                    df_reset_test = self.df_test.reset_index().set_index([c for c in self.indcols if c != "data_tag"])
                    self.df_test = df_reset_test.join(df_temp, how="inner").reset_index().set_index(self.indcols)
            else:
                if self.all_datatags:
                    df_reset = self.df.reset_index()
                    self.df = df_reset.join(df_temp, on='subID').set_index(self.indcols)
                    if self.df_test is not None:
                        df_reset_test = self.df_test.reset_index()
                        self.df_test = df_reset_test.join(df_temp, on='subID').set_index(self.indcols)
                else:
            ...
            self.tsv_name = f"{self.tsv_name}_{os.path.splitext(os.path.basename(tsv_file))[0]}"
            
        # if bool(self.embH5) and self.instance_merge: #TODO: check if this is necessary
        #     self.df = self.df.reset_index().rename({"index":"subID"}, axis=1).set_index(self.indcols)
        #     self.df_test = self.df_test.reset_index().set_index(self.indcols) if self.df_test is not None else None

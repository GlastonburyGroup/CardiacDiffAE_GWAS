...
from utils import remove_high_vif_features

class MLAnalyses:
    def __init__(self, args):
        self.__dict__.update(args)
        #self.res_collect = defaultdict(dict)
        ...
        if bool(self.res_path):
            ...
            self.df.drop(columns='FID', inplace=True) #For now, this is fine. If we want to do multi-modal, we need to re-consider this
            ...
            if bool(self.res_heldout_path):  # held-out test subjects (if present)
                ...
                self.df_test.index = self.df_test.index.astype(int)
                self.df_test.drop(columns='FID', inplace=True) #For now, this is fine. If we want to do multi-modal, we need to re-consider this
                self.df_test = self.df_test.astype(float)
        ...
        self.df = self.df[self.latent_factors]
        if self.df_test is not None:
            self.df_test = self.df_test[self.latent_factors]

        # Covariates
        df_temp = pd.read_table(self.cov_path).set_index('IID').rename(columns=(lambda x: 'BinCAT_' + x if x in ('Sex', 'Genotype_Batch') else ('CAT_' + x if x in ('MRI_Visit', 'MRI_Centre') else x))).rename(columns=(lambda x: x + '_COV'))  # read file of covariates
        self.df = self.df.join(df_temp, how="inner")  # limiting the train participants to those having valid covariates
        self.df_test = self.df_test if self.df_test is None else self.df_test.join(df_temp, how="inner")  # limiting the test participants to those having valid covariates
        ...
        if bool(self.path_norm) and bool(self.attribute_norm):  # normalizing attribute
            normal_attributes = []
            ...
            for c in self.continuous_attributes:
                c_norm = c + "_norm"
                self.df[c_norm] = self.df[c] / self.df[self.attribute_norm]  # normalize
                if self.df_test is not None:
                    self.df_test[c_norm] = self.df_test[c] / self.df_test[self.attribute_norm]  # normalize
        ...
        if bool(self.attributes_assoc):
            self.attributes_assoc = self.attributes_assoc.split(',')
            self.df_association = self.df[self.attributes_assoc + self.latent_factors]
    ...
    def eval_logistic(self, model, features, X_train, X_test=None, y_test=None, is_binary=False, exp_type="", level_type=""):
        if X_test is not None:
            ...
            auc = roc_auc_score(y_test, y_prob[:, 1]) if is_binary else roc_auc_score(y_test, y_prob, multi_class="ovr")
            if bool(self.out_path) and bool(exp_type):
                ...
                self.res_collect[exp_type][level_type]['AUC'] = auc
            else:
    ...
    def association(self, X_train, y_train, exp_type, level_type):
        for latent_factor in self.latent_factors:  # loop through all the latent factors
            X = X_train[self.attributes_assoc].join(y_train, how="inner", rsuffix="_")
    ...
    def RFE_linaer_regression(self, X_train, X_test, y_train, y_test, exp_type, level_type):
        for n_feat in range(2, len(self.latent_factors)):
            model = LinearRegression(n_jobs=self.n_jobs)
    ...
    def train_test(self, X_train, X_test, y_train, y_test, target_attribute, exp_type, level_type):
        if self.df_add is not None:  # prediction with additional features is required
            df_add_train = self.df_add.loc[list(set(self.df_add.index).intersection(set(X_train.index))), :].copy()  # train participants
            df_add_test = self.df_add.loc[list(set(self.df_add.index).intersection(set(X_test.index))), :].copy()  # test participants
...
def process_arguments():
    ...
    # parser.add_argument('--res_path', action="store", default="/project/ukbblatent/Out/Results/F20208_heart_1Ses_time2slc_MskCrop128_mulseed_1701_3D_128fold0_prec16-mixed_DiffAE/Output_fullDSV2", help="Folder containining the results of the model, emb.h5.")
    # parser.add_argument('--merged_path', action="store", default="/group/glastonbury/soumick/GWAS/merged_latents_models/latents_MCCA_mergeWithAvg/5Seeds_f0_DiffAEFP16_128_Crop3D", help="Folder containining the results of a merge, merged_latents_raw.tsv.")
    ...
    parser.add_argument('--cov_path', action="store", default="/project/ukbblatent/soumick/scratch/andrea_20240207/covariates/cov_newset_chp_F20208_Long_axis_heart_images_DICOM_H5v3_NOnoise.tsv", help="Fully-qualified path of the tsv file containining the basic covariates.")
    parser.add_argument("--attributes_assoc", action="store", default="Age_COV,BinCAT_Sex_COV,BSA_COV,CAT_MRI_Centre_COV,MRI_Date_COV", help="Comma-separated list of the (correction) covariates for the association analysis. Leave it blank if association analysis is not desired.")
    
    parser.add_argument('--model_tag', action="store", default="prova", help="Tag to identify the model.")

    parser.add_argument('--out_path', action="store", default="/project/ukbblatent/soumick/ML_analyses/F20208v2/v0.9.6_sixth_basket", help="Path to store the results. Make it blank if if it's desired to not store the results, rather just print them on the console.")
    
    parser.add_argument('--path_tsvs', action="store", default="/project/ukbblatent/soumick/processed_clinical_data/F20208v2/v0.9.6_sixth_basket", help="Location of the tsv files, to be used for analyses. Leave it blank if fully-qualified paths are supplied in --tsv_files.")
    parser.add_argument('--tsv_files', action="store", default="baseline_bodymeas;clinicaldata_aortameas;clinicaldata_cardiacfunc;clinicaldata_negcontrol", help="Coma-separated list of tsv files - to be joined and considered. To consider in one analysis, seperate by coma, to be considered in seperate ones, seperate by semicolon.")
    
    parser.add_argument('--predictors2ignore', action="store", default="", help="Coma-separated list of predictors to ignore.")  #CAT_Ethnicity,Standard_PRS_for_cardiovascular_disease_(CVD).0.0
    parser.add_argument('--attributes2ignore', action="store", default="CAT_Ethnicity", help="Coma-separated list of attributes to ignore.") #Reasons for the default vals: Birth_Month makes no sense, Smoking_Imaging is considered, Ethnicity is mostly white, BMI_Imaging is considered, MRIvisit shouldn't have any impact (but we should check it out in the future)

    parser.add_argument("--path_norm", action="store", default="/project/ukbblatent/soumick/processed_clinical_data/F20208v2/v0.9.6_sixth_basket/clinicaldata_cardiacfunc.tsv", help="Fully-qualified file path of the normalizing attribute (i.e., continuous targets are divided by this attribute). Leave it blank if target normalization is not desired.")
    parser.add_argument("--attribute_norm", action="store", default="LV_end_diastolic_volume", help="Name of the normalizing attribute to retrieve (i.e., continuous targets are divided by this attribute). Leave it blank if target normalization is not desired.")
    ...
    parser.add_argument('--remove_with_VIF', action=argparse.BooleanOptionalAction, default=True, help="Whether to remove VIFs.")
    ...
    #parser.add_argument('--test_size', action="store", type=float, default=0.25, help="Percent of the dataset to create the test subset.")
...
def main():
    ...
    for tsv_file_combo in tsv_file_combos:
        ...
        col_vif_folds, X_scaled_train_folds, X_scaled_test_folds = [None for i in range(args.n_folds)], [None for i in range(args.n_folds)], [None for i in range(args.n_folds)]
        col_vif_dataset, X_scaled_dataset, X_heldout_scaled_dataset = None, None, None
        scaling_completed, vif_completed = False, False
        
        for target_attribute in tqdm(analyser.attributes): 
            #y = analyser.df[target_attribute]
            ...
            if bool(analyser.attributes_assoc):
                analyser.association(X_train=analyser.scale_features(X_train=analyser.df_association.copy(), X_test=None)[0] if args.use_feature_scaling else analyser.df_association.copy(), y_train=analyser.scale_target(y_train=y, y_test=None)[0] if args.use_target_scaling else y, exp_type=f"{target_attribute}_Assoc_dataset", level_type="Train")
            ##### With held-out test (if present, otherwise only train)
            if args.use_feature_scaling:
                if not scaling_completed:
                    X, X_test = analyser.scale_features(X_train=X, X_test=X_test)
                    X_scaled_dataset = X.copy()
                    X_heldout_scaled_dataset = X_test if X_test is None else X_test.copy()
                else:
                    X = X_scaled_dataset.copy()
                    X_test = X_heldout_scaled_dataset if X_heldout_scaled_dataset is None else X_heldout_scaled_dataset.copy()
            ...
            if args.remove_with_VIF:
                if not vif_completed:
                    print('\n--- Remove with VIF, full dataset')
                    X = remove_high_vif_features(X, threshold=10)
                    col_vif_dataset = X.columns
                else:
                    X = X[col_vif_dataset]
                X_test = X_test if X_test is None else X_test[col_vif_dataset]
                analyser.latent_factors = col_vif_dataset

            analyser.train_test(X_train=X, X_test=X_test, y_train=y, y_test=y_test, target_attribute=target_attribute, exp_type="dataset", level_type=("heldoutTest" if X_test is not None else "Train"))


            X = analyser.X.copy()
            analyser.latent_factors = X.columns
            y = analyser.y[target_attribute].copy()
            ##### k-fold (stratified on target if categorical)
            #X_train, X_test, y_train, y_test = train_test_split(analyser.X, y, test_size=args.test_size, random_state=1701)
            ...
            for fold, (train_index, test_index) in enumerate(kf.split(np.zeros(len(y)) if target_attribute in analyser.categorical_attributes else X, y)):
                ...
                # Scalers and Remove with VIF fit on train data only (test data gets only transformed)
                if args.use_feature_scaling:
                    if not scaling_completed:
                        X_train, X_test = analyser.scale_features(X_train=X_train, X_test=X_test)
                        X_scaled_train_folds[fold], X_scaled_test_folds[fold] = X_train.copy(), X_test.copy()
                    else:
                        X_train, X_test = X_scaled_train_folds[fold].copy(), X_scaled_test_folds[fold].copy()
                if args.use_target_scaling and (target_attribute in analyser.continuous_attributes):
                ...
                if args.remove_with_VIF:
                    if not vif_completed:
                        ...
                        col_vif_folds[fold] = X_train.columns
                    else:
                        X_train = X_train[col_vif_folds[fold]]
                    X_test = X_test[col_vif_folds[fold]]
                    analyser.latent_factors = col_vif_folds[fold]

                analyser.train_test(X_train=X_train, X_test=X_test, y_train=y_train, y_test=y_test, target_attribute=target_attribute, exp_type="CV", level_type=f"Fold_{fold}")

            scaling_completed, vif_completed = True, True

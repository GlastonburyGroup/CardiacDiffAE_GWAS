...
def getARGSParser():
    ...
    parser.add_argument('-ph', '--path_phenotypes', type=str, default="/group/glastonbury/GWAS/F20208v3_DiffAE/select_latents_r80/nNs_Qntl_INF30_DiffAE128_5Sd_r80_discov_fullDSV3/processed.tsv", help='Path to the phenotype file. [Default: Blank >> They will be generated using the following parameters.]') #If the phenotype file is available, that can directly be provided
    ...
    parser.add_argument('-n', '--projectName', type=str, default="DiffAE128_5Sd_r80_discov_SexInteract", help='To be used as the project name for the GWAS and other processed data. [Default: "" (trainID is being used)]')    
    ...
    parser.add_argument('-nl', '--nLatents', type=int, default=-1, help='Number of latent factors. Set it to -1 to use all the clumns (except FID and IID) from the final phenotype file [Default: -1]')
    ...
    parser.add_argument('-is', '--in_train_subfold', type=str, default="", help='Subfolder inside the model (trainID) folder containing the embeddings. [Default: "" (Output_fullDSV2)]')
    ...
    parser.add_argument("--interaction_cov", type=str, help="Optional param, to specify interaction covariate", default="Sex")
...
def prep_args_conf(args, args4conf):
    if bool(args['selectiveLatents']):
        ...
        if args['nLatents'] == -1:
            pheno = pd.read_table(args['path_phenotypes'] if bool(args['path_phenotypes']) else f"{args['out_path']}/processed.tsv")
            latents = ",".join([c for c in pheno.columns if c not in ['FID', 'IID']])
        else:
    ...
    if bool(args['interaction_cov']):
        args4conf['params']['interaction_cov'] = args['interaction_cov']
...
if __name__ == "__main__":
    ...
    if bool(args['path_phenotypes']):
        ...
        if args['in_train_subfold'] == "":
            args['in_train_subfold'] = "Output_fullDSV2"
        ...
        if args['out_path'] == "":
            ...
            if args['out_subdir_tag'] != "":
                ...
                args['out_path'] = f"{args['out_path']}_fullDSV2"
        else:

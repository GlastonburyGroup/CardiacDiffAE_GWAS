...
def getARGSParser():
    ...
    parser.add_argument('-t', '--trainID', default="", type=str, help='Train ID (Name of the model). Will also be used as the project name for the GWAS (if projectName not specified).')
    parser.add_argument('-ph', '--path_phenotypes', type=str, default="/group/glastonbury/emanuele/GWAS/inferred_abdominal_traits/values_raw.tsv", help='Path to the phenotype file. [Default: Blank >> They will be generated using the following parameters.]') #If the phenotype file is available, that can directly be provided
    
    parser.add_argument('-dsv', '--dsV', type=str, default="", help='Version of the dataset to be used [This also is the value of ~ in subsequent arguments]. [Default: V2. For V1, leave it blank.]')
    
    parser.add_argument('-n', '--projectName', type=str, default="DINOAbdTrtFullInfer", help='To be used as the project name for the GWAS and other processed data. [Default: "" (trainID is being used)]')    
    ...
    parser.add_argument('-sl', "--selectiveLatents", type=str, default="L_kidney_volume,R_kidney_volume,liver_volume,lung_volume,spleen_volume,subcutaneus_fat_volume,visceral_fat_volume", help="Comma separated list of latent factors to be used for the GWAS. [Default: None]")
    ...
    parser.add_argument("--txt_subIDs", help="Path to a text file containing coma-seperate list of subIDs (This has priority over subIDs)", default="")
    ...
    parser.add_argument("-cov", "--covariates", type=str, help="Path to the covariates file", default="/group/glastonbury/emanuele/GWAS/inferred_abdominal_traits/covariatesV1.tsv")
    parser.add_argument("-ns", "--no_noise", action=argparse.BooleanOptionalAction, default=False, help='Whether to avoid noisy images (using the "_NOnoise" version of the covariates file)')
    
    parser.add_argument("--cov_cols", type=str, help="Coma-seperated list of covarites columns. Leave it blank to use the deafault options. Only if this is supplied, cov_cat_cols and max_cat_levels will be used, and in that case, they are necessary.", default="")
    parser.add_argument("--cov_cat_cols", type=str, help="Coma-seperated list of categorical covarites columns", default="")
...
def prep_args_conf(args, args4conf):
    ...
    if bool(args['cov_cols']):
        args4conf['params']['covariates_columns'] = args['cov_cols']
        args4conf['params']['covariates_cat_columns'] = args['cov_cat_cols']
        args4conf['params']['maxCatLevels'] = args['max_cat_levels']

    return args4conf

def create_conf(args, args4conf, tag=""):
    conf = ConfFile("GWAS/template_DXN.conf")
    ...
    if bool(args['path_phenotypes']):
        ...
        if args['out_path'] == "":
            ...
            if args['out_subdir_tag'] != "":
                ...
                args['out_path'] = f"{args['out_path']}_fullDS{args['dsV']}"
        else:

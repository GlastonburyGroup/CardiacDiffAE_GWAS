...
def getARGSParser():
    ...
    parser.add_argument('--filter_alfreq', action=argparse.BooleanOptionalAction, default=False, help="Whether to filter the GWAS tophits based on the allele frequency (>0.01). [Only if process_dir_tophits is False]")
    ...
    parser.add_argument('--test_mode', type=str, default="ADD", help="Which test mode to consider for plotting. Typically for GWAS or EWAS, ADD is used.")
    ...
    parser.add_argument("--n_ind_tests", type=int, default=-1, help="N independent tests performed, to be used to calculate the Bonferroni corrected significance level. If set to 0, then the significance level will be used as is. If set to -1, then will be computed based on the number of GWASes performed")
    ...
    parser.add_argument("--qq_bins", type=str, default="0.01,0.05,0.1,0.25,0.5", help="Bins for stratifying the QQ plots.")
    parser.add_argument("--colour_qq_bins", type=str, default="#f05f4e,#f0ad4e,#5cb85c,#5bc0de", help="Bins for stratifying the QQ plots.")
    
    parser.add_argument("--plot_mode", type=str, default="qqm", help="Type of plot: mqq,qqm,qq,m")
...
def process_hits(g, args, processed_tophits):
    with gzip.open(g, 'rt') as f:
        df = pd.read_table(f)
        if args.test_mode != "ADD":
            df = df[df["TEST"] == args.test_mode]
    ...
    if len(df_filtered) > 0 and args.filter_maf:
        df_filtered['MAF'] = df_filtered['A1FREQ'].apply(calculate_maf) 
        df_filtered = df_filtered[df_filtered['MAF'] > 0.01]
    if len(df_filtered) > 0 and args.filter_toploci_mode:
        top_loci = pd.read_table(g.replace(os.path.basename(g), f"toploci/{os.path.basename(g).split('.')[0]}.toploci.tsv"))
...
def plot_item(g, args, corrected_sig_level, usecols):
    ...
    if args.gene_plot:
        mysumstats.plot_mqq(
            mode=args.plot_mode, colors = args.colours, scaled=True, sig_level=args.sig_level, sig_line_color=args.colour_sigline,
            ...
            saveargs={"dpi":args.dpi, "facecolor":"white"}
        )
        plt.close()
...
if __name__ == "__main__":
    ...
    # Bonferroni correction: adjust based on the number of GWASs
    if args.n_ind_tests == -1:
        args.n_ind_tests = len(gwas_outs)
    elif args.n_ind_tests == 0:
        args.n_ind_tests = 1
    corrected_sig_level = args.sig_level/args.n_ind_tests 
    ...
    if not bool(args.path4res):
        args.path4res = f"{os.path.dirname(args.path2gwasout)}/vis"
    ...
    if args.test_mode != "ADD":
        subdir += f"_{args.test_mode.replace('ADD-','')}"
    if bool(subdir):

...
then
   ...
   IFS=';' read -r -a jobpart_IDs <<< "$jobparts"
   for jobpart_ID in "${jobpart_IDs[@]}"
   do
      #Create the QQ and Manhattan plots (with and without filtering based on the top loci)
      sbatch §§NODEEXCLUDE§§ /home/soumick.chatterjee/gwasvis.sh --args "--path2gwasout $path2gwas/$jobpart_ID"
      sbatch §§NODEEXCLUDE§§ /home/soumick.chatterjee/gwasvis.sh --args "--path2gwasout $path2gwas/$jobpart_ID --filter_toploci_mode 1"

      #Create the combined toploci file
      lastJobID=$(sbatch  -J ovrlp §§NODEEXCLUDE§§ /home/soumick.chatterjee/cpurun.sh --root /group/glastonbury/soumick/MyCodes/GitLab/tricorder/ --programme GWAS/analyses/overlap.py --args "--path2gwasouts $path2gwas/$jobpart_ID" | awk '{print $NF}')

      #Fetch the LD traits from LDLink and create the wordclouds
      sbatch --dependency=afterok:$lastJobID §§NODEEXCLUDE§§ /home/soumick.chatterjee/ldtrait.sh --args "--toplocifile $path2gwas/$jobpart_ID"
   done
else
   ...
   part_phenotypes="§§PARTPHENOLIST§§"
   IFS=';' read -r -a part_phenotypes <<< "$part_phenotypes"
   for iP in "${!part_phenotypes[@]}"
   do   
      phenotypes=${part_phenotypes[$iP]}
      IFS=',' read -r -a phenotypes <<< "$phenotypes"

      jobpart_ID=${jobpart_IDs[$iP]}

      path2sumstats="$path2gwas/$jobpart_ID/results/gwas"
      ...
      then
         ...
         sbatch --dependency=afterok:$toplociJob §§NODEEXCLUDE§§ /home/soumick.chatterjee/gwasvis.sh --args "--path2gwasout $path2gwas/$jobpart_ID --filter_toploci_mode 1"

         #Create the combined toploci file
         lastJobID=$(sbatch --dependency=afterok:$toplociJob -J ovrlp §§NODEEXCLUDE§§ /home/soumick.chatterjee/cpurun.sh --root /group/glastonbury/soumick/MyCodes/GitLab/tricorder/ --programme GWAS/analyses/overlap.py --args "--path2gwasouts $path2gwas/$jobpart_ID" | awk '{print $NF}')

...
def process_item(metrics_path, args, train_set, val_set, pheno_df=None):    
    ...
    if args.n_recon_GIFs > 0:
        recons = []
        with h5py.File("/project/ukbblatent/Out/Results/F20208_heart_1Ses_time2slc_MskCrop128_V2_3D_ph1_ce1rc1va1_4ChTrans128fold0_precbf16-mixed_pythaemodel-custom_ultra_cevae/Output_fullDS/recon.h5", "r") as f:
            keys = sorted(f.keys())
...
def getARGSParser():
    ...
    parser.add_argument("--res_path", help="path to the trainings", default="/project/ukbblatent/Out/Results")
    parser.add_argument("--trainID", help="Leave it blank if all the trainings in the res_path are to be analysed", default="")

    parser.add_argument("--pheno_path", help="path to the file to be used for evaluating phenotypes", default="/group/glastonbury/sara/gwas/input/cardiac_pheno_35k.tsv")
    # parser.add_argument("--valcols", help="path to the file to be used for evaluating phenotypes", default="LV_EjectionFraction,LV_endDiastolicVol,LV_endSystolicVol,LV_StrokeVol,Cardiac_Output,Cardiac_Index,Avg_HeartRate")
    parser.add_argument("--valcols", help="path to the file to be used for evaluating phenotypes", default="LV_endDiastolicVol,LV_endSystolicVol,LV_StrokeVol")

    parser.add_argument("--split_csv_path", help="path to the fold CSV", default="/project/ukbblatent/datasets/F20208_Long_axis_heart_images_DICOM_H5/5folds_1Ses_100DS_60Trn_10Val_30Tst_1701seed.csv")
